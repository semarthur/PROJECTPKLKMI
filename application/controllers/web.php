...
class Web extends CI_Controller {
	...
	function export(){
		...
		$excel->setActiveSheetIndex(0)->setCellValue('B1', "DATA STATUS"); 
		...
		$filename = "REQ_FORM".date("Y_m_d").'.xlsx';
	...
	function export_done(){
		...
		$data = $this->db->query("select * from form_done")->result();
		$no = 1; 
		$numrow = 4;
		foreach($data as $row){ 
			$excel->setActiveSheetIndex(0)->setCellValue('A'.$numrow, $no);
		...
		$filename = "REQ_FORM_DONE".date("Y_m_d").'.xlsx';
